...
from sqlalchemy import create_engine # Giữ lại import này nếu cần
...
def hien_thi_quan_ly_user(config):
    ...
    with tab3:
        ...
        with col_change_1: user_to_change = st.selectbox("Chọn tài khoản:", list_all_users, key="sel_change")
        with col_change_2:
            st.write("") 
            st.write("") 
...
def set_state(name):
    for key in ['search', 'loc', 'han', 'bieu', 'chuan', 'ai', 'admin_data', 'admin_user', 'admin_log']:
...
def hien_thi_loc_loi(df, ten_cot):
    ...
    rong = ['nan', 'none', 'null', '', '0']; df_loc = df[col_chuan.isin(rong)]
    if not df_loc.empty:
...
def hien_thi_kiem_tra_han(df, ten_cot_ngay):
    ...
    try:
        ...
        if not ds_het.empty:
            st.subheader("🔴 Danh sách Hết Hạn"); excel_het = tao_file_excel(ds_het)
            st.download_button("📥 Tải Hết Hạn", excel_het.getvalue(), "het_han.xlsx"); st.dataframe(ds_het.head(500), hide_index=True)
        if not ds_sap.empty:
            st.subheader("⚠️ Danh sách Sắp Hết"); excel_sap = tao_file_excel(ds_sap)
            st.download_button("📥 Tải Sắp Hết", excel_sap.getvalue(), "sap_het.xlsx"); st.dataframe(ds_sap.head(500), hide_index=True)
...
def hien_thi_bieu_do_tuong_tac(df, ten_cot):
    ...
    if event and event['selection']['points']:
        ...
        log_action(st.session_state["username"], "Click Biểu Đồ", f"Xem chi tiết: {gia_tri_chon}")
        hien_thi_uu_tien(df[df[ten_cot] == gia_tri_chon])

def hien_thi_chatbot_thong_minh(df):
    log_action(st.session_state["username"], "Xem Chatbot", "")
    ...
    if prompt := st.chat_input("Nhập yêu cầu..."):
        ...
        log_action(st.session_state["username"], "Chat AI", prompt); with st.chat_message("assistant"):
            ...
            try:
                ...
                if len(ten) > 1:
                    df_res = df_res[df_res['hoTen_khongdau'].str.contains(ten)]; filters.append(f"Tên chứa: **{ten}**")
...
def main():
    ...
    # FIX: ĐÃ BỎ UNPACKING TẠI ĐÂY
    authenticator.login(location='main') 

    if st.session_state["authentication_status"]:
        # Log Đăng nhập (Chỉ ghi 1 lần)
        ...
        df = nap_du_lieu_toi_uu() # Dòng này gọi hàm load data
